...
const Navbar = () => {
  ...
  const handleSearch = async () => {
    try {
      const response = await fetch(
        `http://192.168.60.254:3001/search?query=${searchTerm}`,
        {
          method: "GET",
          headers: {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      const response = await fetch(
        `http://192.168.60.254:3001/notifications/${user._id}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    try {
      await Promise.all(
        notifications.map((notification) =>
          axios.put(
            `http://192.168.60.254:3001/notifications/read/${notification._id}`,
            {},
            {
              headers: {
                Authorization: `Bearer ${token}`,
